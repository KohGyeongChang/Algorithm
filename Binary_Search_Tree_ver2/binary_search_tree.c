...
treeNode* buildBinaryTree()
{
    ...
    if ( pLeft ) {
	    ...
	    if ( pRight ) {
			...
			pLeft2 = addLeftChild(pRight, 85, "I");
		    pRight2= addRightChild(pLeft2, 88, "M");

			addLeftChild(pRight2, 87, "N");
		    addRightChild(pRight2, 89, "O");
...
void deleteTwo(treeNode* pTreeNode, treeNode* pParentNode, treeNode* pFoundNode)
{
	...
	treeNode* delRight = pFoundNode->pRiChild;
	if ( delRight->pLeChild == NULL ) {

		if ( pParentNode->pLeChild == pFoundNode ) {
			pParentNode->pLeChild = pFoundNode->pRiChild ;
			pParentNode->pLeChild->pLeChild = pFoundNode->pLeChild ;
		}
		else {
			pParentNode->pRiChild = pFoundNode->pRiChild ;
			pParentNode->pRiChild->pLeChild = pFoundNode->pRiChild ;
		}
	}
	else {

		while ( delRight->pLeChild->pLeChild ) {
			delRight = delRight->pLeChild;
		}
		replaceNode = delRight->pLeChild;
		delRight->pLeChild = replaceNode->pRiChild;

		if ( pParentNode->pLeChild == pFoundNode ) {
			pParentNode->pLeChild = replaceNode;
		}
		else {
			pParentNode->pRiChild = replaceNode;
		}
		replaceNode->pLeChild = pFoundNode->pLeChild;
		replaceNode->pRiChild = pFoundNode->pRiChild;
	}
